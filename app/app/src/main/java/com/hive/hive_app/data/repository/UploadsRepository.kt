...
class UploadsRepository @Inject constructor(
    ...
    suspend fun uploadForumEventImage(context: Context, uri: Uri): Result<String> {
        return try {
            val multipart = withContext(Dispatchers.IO) {
                val contentResolver = context.contentResolver
                val mimeType = contentResolver.getType(uri) ?: "image/*"
                val mediaType = mimeType.toMediaTypeOrNull()
                val fileName = getDisplayName(context, uri) ?: "event.jpg"
                val bytes = contentResolver.openInputStream(uri)?.use { it.readBytes() }
                    ?: throw IllegalStateException("Unable to read image")
                val requestBody = bytes.toRequestBody(mediaType)
                MultipartBody.Part.createFormData("file", fileName, requestBody)
            }
            val response = uploadsApi.uploadForumEventImage(multipart)
            val body = response.body()
            if (!response.isSuccessful || body == null) {
                return Result.failure(IllegalStateException("Image upload failed"))
            }
